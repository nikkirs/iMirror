...
weather_api_token = "bbe6727ec19b9fdf3683a54b4340ba4d"
wit_ai_token = "Bearer ABXJ4SR3A7NK3CUZFAELZIRHANYOYI6B"
        ...
        if nlu_entities is not None:
            if 'datetime' in nlu_entities:
                if 'grain' in nlu_entities['datetime'][0] and nlu_entities['datetime'][0]['grain'] == 'day':
                    dtime_str = nlu_entities['datetime'][0]['value']
                    dtime = dateutil.parser.parse(dtime_str)
                    if current_dtime.date() == dtime.date():
